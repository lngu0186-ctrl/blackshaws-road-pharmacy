  ...
  return (
    <footer className="relative overflow-hidden bg-[var(--color-navy-deep)] text-white">
      ...
      <div className="container-custom py-16 md:py-20">
        <Reveal className="mb-14 grid gap-10 rounded-[32px] border border-white/10 bg-white/5 p-8 backdrop-blur-sm lg:grid-cols-[1.3fr_0.8fr_0.8fr_0.9fr]">
          <div>
        ...
        <div className="grid gap-10 lg:grid-cols-[1fr_0.7fr]">
          ...
          <Reveal variant="scale" delay={120} className="rounded-[28px] bg-white/6 p-6">
            ...
            <div className="mt-5 flex flex-wrap gap-3">
              <a href="tel:0393913257" className="inline-flex rounded-full bg-red-600 px-5 py-3 text-sm font-semibold text-white transition-transform hover:scale-[1.03] active:scale-[0.98]">Call (03) 9391 3257</a>
    ...
    <div>
      ...
      <ul className="mt-6 space-y-3 text-sm text-white/76">
        {links.map((link) => (
          <li key={link.label}>
            {link.action === 'upload' ? (
              <button type="button" onClick={openUpload} className="footer-link inline-flex items-center gap-2 hover:text-white">
                {link.label}
              </button>
            ) : (
              <a href={link.href} target={link.target} rel={link.target ? 'noopener noreferrer' : undefined} className="footer-link inline-flex items-center gap-2 hover:text-white">
                {link.label} {link.target && <ArrowUpRight className="h-3.5 w-3.5" />}
              </a>
            )}
          </li>
